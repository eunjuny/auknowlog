...
기준 표본도 사용자가 독립적으로 다시 판정할 수 있다. 이 결과는 `reviewer_verdict`와 `reviewed_at`에 초기 `reference_verdict`와 분리해 저장한다. 초기 라벨을 덮어쓰지 않으므로, 표본 설계자의 판단과 사용자 검토의 일치 여부를 나중에 감사할 수 있다. 이 사용자 검토는 기준 데이터셋의 보조 검증 기록이며, 실제 학습 이력 문제 쌍의 사람 검증 지표와는 합산하지 않는다.
...
대시보드는 품질 평가 화면의 원본 판정 목록을 복제하지 않고, 저장된 평가 결과를 읽기 전용 지표로 요약한다. 두 차트 모두 평가 실행 시 추가 AI 호출이나 임베딩 호출을 만들지 않는다.

- **임계값별 정밀도·재현율·F1**: 사람이 `DUPLICATE`·`RELATED`·`DISTINCT`로 확정한 문제 쌍만 사용한다. x축은 0.75~0.95 임계값, y축은 각 성능 지표(%)다. 표본이 없으면 그래프 대신 표본 수집 안내를 표시한다.
- **목표 누락률·문항 일치도**: `AI 잠정`과 `사람 검증` 막대를 분리한다. 누락률은 낮을수록, 일치도는 높을수록 좋다. 사람 검증 막대가 비어 있으면 AI 평가 초안만 존재하는 상태다.

이 분리는 AI가 낸 수치를 검증 완료된 성과처럼 보이게 하지 않기 위한 정책이다. 추천 임계값도 최소 30개 사람 표본과 정밀도 90% 조건을 만족할 때만 표시하며, 차트 결과가 운영 임계값을 자동으로 바꾸지는 않는다.
...
| `PUT /api/quality-evaluations/datasets/{id}/samples/{order}/review` | 없음 | 기준 표본의 사용자 독립 판정을 초기 라벨과 분리해 저장 |
...
| `GET /api/dashboard` | 없음 | 위 검증 지표를 대시보드 차트용 읽기 모델로 함께 조회 |
