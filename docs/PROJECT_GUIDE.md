...
| 메뉴 | 기능 | 필요한 이유 |
| --- | --- | --- |
| 대시보드 | 풀이량·정답률·오늘 복습·약점 추천과 AI 호출 지표를 한 화면에 제공 | 학습 상태와 AI 품질/비용 문제를 분리해 판단한다. |
...
- AI는 형식화된 초안을 만들지만, 한도·중복·의존 관계·저장은 서버 규칙으로 재검증한다.
...
sequenceDiagram
    ...
    B-->>F: 점수·정답·해설·다음 복습 정보
```

정답과 해설은 생성 응답에서 화면으로 보내지지 않는다. 사용자가 답안을 제출하면 서버가 정답과 비교하고, 풀이 기록과 복습 예약을 함께 저장한다. 이 경계가 없으면 브라우저 개발자 도구에서 정답을 확인할 수 있고 정답률·복습 데이터의 신뢰도도 떨어진다.
...
필수 목표 문항을 모두 다룬 뒤에는 자동으로 다음 소주제를 열지 않는다. 사용자는 같은 소주제를 추가 학습하거나 `다음 단계로 진행`을 확정한다. 이 확정 전에는 다음 단계가 `LOCKED`이고, 추가 학습은 목표 전체를 다시 순환 배정한다. 목표 충족과 진행 확정은 `learning_roadmap_step.advance_confirmed_at`으로 분리하므로, 진행률 100%가 곧 학습 순서 해제를 뜻하지 않는다. 상세 상태와 Git 저장 구조는 [로드맵 진행·Git 학습 노트](ROADMAP_PROGRESS_AND_GIT_EXPORT.md)를 참고한다.
...
저장한 본문은 SHA-256으로 중복을 막고 `source_chunk`로 나눠 보관한다. AI 로드맵 생성 시 앞에서부터 최대 8개 청크만 문맥으로 전달하며, 외부 문서의 지시문은 실행하지 않는 데이터로 취급한다. 상세 제한과 위협 모델은 [학습 자료 수집 보안 설계](SOURCE_INGESTION_SECURITY.md)를 참고한다.
...
스키마는 Flyway V1~V16으로 관리한다. V13은 학습 목표와 문항-목표 연결을, V14는 품질 평가 실행·문제 쌍·목표 검토 데이터를, V15는 분리된 중복 평가 기준 데이터셋을, V16은 로드맵 단계의 목표 충족과 진행 확정 상태를 분리하는 시각을 추가한다. JPA의 자동 DDL 생성을 사용하지 않고 애플리케이션 시작 시 스키마를 검증한다.
...
| 스키마 관리 | Flyway | 환경마다 같은 순서의 DB 변경과 검증 가능한 이력 | V1~V16 마이그레이션 |
| AI 생성·평가 | OpenAI Responses API, Structured Outputs | 퀴즈·로드맵의 JSON 계약을 제한하고, 품질 평가도 모든 문항 ID·판정·확신도를 구조화해 서버에서 재검증 | 문제·로드맵 초안, 목표별 문제 생성, 명시적 목표 품질 평가 |
...
- Prometheus(선택): `docker compose --profile monitoring up -d`, `http://127.0.0.1:9090`
...
| 실제 DB 통합 | `cd backend && ./gradlew integrationTest` | Testcontainers PostgreSQL 16 + pgvector, Flyway V1~V16, 학습 목표·품질 평가 FK, 로드맵 진행 확정 상태, 기준 데이터셋 vector(512), HNSW, 평가 후보 SQL, 차원 불일치 거부 |
