...
개발용 샘플 화면은 Vite의 `/daily-tech-learning-sample.html` 한 경로로만 제공한다. `docs` 디렉터리를 통째로 공개하지 않으며, `GET`·`HEAD` 이외의 요청은 거부한다. 원격 접속 메일에는 기본 URL과 이 경로를 붙인 직접 URL을 함께 보낸다. 직접 URL로 열면 로그인 후에도 이 화면으로 다시 이동한다. 터널이 종료되었거나 맥의 Vite·인증 프록시·cloudflared가 중단되면 이 경로도 사용할 수 없다.
