...
| 문제 | 선택 | 적용 방식 | 검증 |
| --- | --- | --- | --- |
| ES `match` 점수는 의미 중복을 정확히 표현하지 못하고 PostgreSQL과 이중 쓰기가 발생 | PostgreSQL + pgvector | 원본 이력과 벡터를 같은 PostgreSQL에 저장하고, 코사인 유사도로 의미 중복 후보를 조회 | 임베딩이 비활성일 때는 정확 해시로 안전하게 동작. PostgreSQL/pgvector 통합은 Docker 가능한 CI에서 Testcontainers로 추가 검증 예정 |
...
| Testcontainers | 로컬 Docker 데몬이 현재 실행되지 않아 이 작업에서 컨테이너 테스트를 수행할 수 없다. | GitHub Actions 또는 Docker 가능 환경에서 pgvector V3 마이그레이션·코사인 검색을 실제 PostgreSQL로 검증할 때. |
